    ...
    <div className="flex flex-col items-center px-3 pb-10">
      ...
      {lastMsg && (
        <div className="mb-3 rounded-full px-5 py-2 font-sans text-sm font-black animate-pulse"
          style={{background:lastMsg.good?"linear-gradient(135deg,#4CAF50,#66BB6A)":"linear-gradient(135deg,#EF5350,#FF7043)",color:"white"}}>
          {lastMsg.text}
        </div>
      )}
        ...
        <div className="text-center mb-6 max-w-sm">
          ...
          <div className="grid grid-cols-3 gap-2 mb-5">
            {["⬆️ Tap UP lane","⬇️ Tap MID lane","⬆️ Tap DOWN lane"].map((t,i)=>(
              <div key={i} className="rounded-lg p-2 text-xs text-center text-white/50" style={{background:"rgba(255,255,255,0.05)"}}>{t}</div>
            ))}
          </div>
          <button onClick={start} className="px-8 py-3 rounded-full font-sans font-black text-sm" style={{background:"linear-gradient(135deg,#FF5722,#FF9800)",color:"white",boxShadow:"0 4px 20px rgba(255,87,34,0.5)"}}>
            🏃 Start Running!
          </button>
        </div>
      )}
      ...
      {running && (
        <div className="relative rounded-2xl overflow-hidden select-none"
          style={{width:"min(520px,100%)",height:240,background:"linear-gradient(180deg,#87CEEB 0%,#98FB98 70%,#5D4037 100%)",border:"2px solid rgba(255,152,0,0.4)"}}>
          ...
          {[0,1,2].map(l=>(
            <button key={l} onClick={()=>setLane(l)}
              ...
              style={{top:`${LANES_Y[l]-8}%`,height:"16%",background:lane===l?"rgba(255,215,0,0.15)":"transparent",color:"rgba(255,255,255,0.4)",zIndex:10}}>
              {lane===l?"→":l===0?"↑":l===2?"↓":"•"}
            </button>
          ))}
          ...
          {[20,35,50,65,80].map((y,i)=>(
            <div key={i} className="absolute h-px bg-white/20" style={{top:`${y}%`,left:"10%",width:`${20+i*10}px`,animation:`speedLine ${0.3+i*0.1}s linear infinite`}}/>
          ))}
        </div>
      )}
        ...
        <div className="flex gap-4 mt-4">
          {[{l:0,label:"Top Lane ↑"},{l:1,label:"Mid Lane →"},{l:2,label:"Bot Lane ↓"}].map(b=>(
            <button key={b.l} onClick={()=>setLane(b.l)}
              ...
              style={{background:lane===b.l?"rgba(255,215,0,0.3)":"rgba(255,255,255,0.05)",border:`1px solid ${lane===b.l?"#FFD700":"rgba(255,255,255,0.1)"}`,color:lane===b.l?"#FFD700":"rgba(255,255,255,0.6)"}}>
              {b.label}
            </button>
          ))}
        </div>
        ...
        <div className="rounded-3xl p-8 text-center max-w-sm w-full mt-4" style={{background:"linear-gradient(135deg,#1a0a00,#2d1500)",border:"2px solid #FF9800",boxShadow:"0 0 60px rgba(255,152,0,0.4)"}}>
          ...
          <div className="grid grid-cols-3 gap-3 my-5">
            {[{l:"Soul Glow",v:`${glow}%`},{l:"Score",v:score},{l:"Distance",v:`${distance}m`}].map(s=>(
              <div key={s.l} className="rounded-xl p-3" style={{background:"rgba(255,152,0,0.15)"}}>
          ...
          <button onClick={start} className="px-8 py-3 rounded-full font-sans font-black text-sm" style={{background:"linear-gradient(135deg,#FF5722,#FF9800)",color:"white"}}>Run Again! 🏃</button>
        </div>
      )}
      <style>{`
