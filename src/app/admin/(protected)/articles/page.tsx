  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="font-display-hi text-3xl text-charcoal">लेख प्रबंधन</h1>
        <Link
      ...
      {loading ? (
        ...
        <p className="font-hindi text-charcoal/50">अभी तक कोई लेख नहीं जोड़ा गया।</p>
      ) : (
          ...
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-charcoal/5">
              {articles.map((a) => (
                <tr key={a._id}>
                  <Td>
                    <p className="font-hindi text-sm font-medium text-charcoal">{a.title}</p>
                    <p className="font-sans text-xs text-charcoal/45">/{a.slug}</p>
                  </Td>
                  <Td>{a.category}</Td>
                  <Td>{a.author}</Td>
                  <Td>
                    <div className="flex items-center gap-3">
                      <Link href={`/admin/articles/${a._id}/edit`} className="text-gold-deep">
                        <Pencil size={15} />
                      </Link>
                      <button onClick={() => handleDelete(a._id)} className="text-red-500">
                        <Trash2 size={15} />
                      </button>
                    </div>
                  </Td>
